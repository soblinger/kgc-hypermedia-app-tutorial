...
export const renderer: Renderer<FocusNodeViewContext<OperationFormViewState>> = {
  ...
  render(pointer) {
    ...
    if (!this.state.locals.shapes) {
      ...
      if ('types' in this.params.core.contentResource!) {
        resource = this.params.core.contentResource._create<RdfResource>(pointer as any)
      } else {
        const rootResource = this.params.core.client?.resources.get(
          this.params.core.contentResource!.id as NamedNode,
        )
      ...
      if (!operation) {
        return html`Operation ${operationType?.value} not found`
      }
